...
/* server.js patch4: 29d 1m limit, 60->29d 5m, no overlap, separated try/catch */
...
/* ✅ Separated backfill, strict 29-day cutoff for 1m */
async function backfillSymbol(symbol) {
  ...
  try {
    const now = new Date();
    const d29 = new Date(now.getTime() - 29 * 24 * 3600 * 1000);
    const d60 = new Date(now.getTime() - 60 * 24 * 3600 * 1000);

    // Backfill 5m (60d → 29d)
    try {
      await backfillRange(symbol, '5m', d60, d29, 6);
      await updateCollectionStatus(`backfill_${symbol}_5m`, 'completed');
    } catch (e) {
      collectionStats.errors++;
      await updateCollectionStatus(`backfill_${symbol}_5m`, 'error', e.message);
    }

    // Backfill 1m (29d → now)
    try {
      await backfillRange(symbol, '1m', d29, now, 6);
      await updateCollectionStatus(`backfill_${symbol}_1m`, 'completed');
    } catch (e) {
      collectionStats.errors++;
      await updateCollectionStatus(`backfill_${symbol}_1m`, 'error', e.message);
    }
...
app.get('/api/status', async (req, res) => {
  try {
    ...
    for (const sym of SYMBOLS) {
      for (const tf of ['1m','5m']) {
        const days = tf === '1m' ? 29 : 60;
        const r = await runSql(
          `SELECT $1 AS symbol, $2 AS timeframe,
...
(async () => {
  try {
    ...
    for (const s of SYMBOLS) {
      for (const tf of ['1m','5m']) {
        const days = tf === '1m' ? 29 : 60;
        const minRows = tf === '1m' ? 25000 : 5000;
        const r = await runSql(
